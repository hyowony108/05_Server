...
@WebServlet("/signUp")
public class ExampleConteroller2 extends HttpServlet{
...
//		-> webapp 폴더가 컨첵스트 루트(/)로 설정되기 때문에,
//		   해당 경로가 webapp 내부 경로로 해석되는 것.
		
//		WEB_INF 폴더에 있는 파일들은 클라이언트가 직접적으로 접근할 수 없는 영역,
//		서블릿을 통해서만 접근 가능
		
		dispatcher.forward(req, resp);
//		RequestDispatcher 객체를 사용하여 현재 요청(req)와 응답(resp)을
//		지정한 JSP 페이지(result.jsp)로 전달하는 작업
//		-> 즉, 현재 서블릿(ExampleConteroller2)에서 처리하던 요청을 result.jsp로 전달하고
//		   제어권을 그곳으로 넘김(== 위임하다)
